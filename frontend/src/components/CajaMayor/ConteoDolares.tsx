...
const ConteoDolares: React.FC<ConteoDolaresProps> = ({ 
  open, 
  onClose, 
  onSaveTotal,
  saldoSistema = 0,
  ...
  // Estado para las denominaciones (valores habituales de dólares)
  ...
  // Función para formatear montos de dólares
  ...
  useEffect(() => {
    const nuevoTotal = denominaciones.reduce((acc, d) => acc + (d.valor * d.cantidad), 0);
    setTotal(nuevoTotal);
  }, [denominaciones]);
                ...
                {denominaciones.map((denom, index) => {
                  ...
                  return (
                    <TableRow key={`USD-${denom.valor}`}>
                      ...
                      <TableCell>
                        $ {formatearMonto(denom.valor * denom.cantidad)}
                      </TableCell>
                    </TableRow>
                  );
                })}
