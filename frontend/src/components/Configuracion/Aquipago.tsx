...
const Aquipago: React.FC = () => {
  // Estados
  const [loading, setLoading] = useState<boolean>(true);
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [cuentasBancarias, setCuentasBancarias] = useState<CuentaBancaria[]>([]);
  const [configActual, setConfigActual] = useState<AquipagoConfig | null>(null);
  const [historialConfig, setHistorialConfig] = useState<AquipagoConfig[]>([]);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const configResponse = await aquipagoService.getLatestConfig();
        if (configResponse) {
        ...
        // Cargar historial
        const historialResponse = await aquipagoService.getConfigHistory();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Guardar configuración
      const response = await aquipagoService.createConfig(data);
  ...
  const handleDownloadContrato = async (id: number) => {
      try {
      const response = await aquipagoService.downloadContrato(id);
      const url = window.URL.createObjectURL(new Blob([response]));
          const link = document.createElement('a');
          link.href = url;
      link.setAttribute('download', `contrato-aquipago-${id}.pdf`);
                ...
                <CardContent>
                  ...
                  {configActual ? (
                    ...
                    <Typography variant="body1" sx={{ color: 'rgba(255, 255, 255, 0.7)' }}>
                      No hay configuración registrada para Aqui Pago.
                    </Typography>
                  )}
                </CardContent>
              </Card>
              ...
              <Card sx={{ backgroundColor: '#1E1E1E', color: 'white', height: '100%' }}>
                <CardContent>
                  ...
                  <Alert severity="info" sx={{ mb: 2, color: 'black' }}>
                    La configuración de Aqui Pago determina la cuenta bancaria donde se depositarán los fondos
                  ...
                  <Box sx={{ mt: 2 }}>
                    ...
                    <Button
                      variant="text"
                      color="primary"
                      onClick={() => window.location.href = '/dashboard/saldos-monetarios/aqui-pago'}
                    >
                      Ir a Balance Aqui Pago
                    </Button>
                  </Box>
