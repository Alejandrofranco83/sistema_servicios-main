...
const WepaUsd: React.FC = () => {
  // Estados
  const [loading, setLoading] = useState<boolean>(true);
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [cuentasBancarias, setCuentasBancarias] = useState<CuentaBancaria[]>([]);
  const [configActual, setConfigActual] = useState<WepaUsdConfig | null>(null);
  const [historialConfig, setHistorialConfig] = useState<WepaUsdConfig[]>([]);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        // Filtrar solo cuentas en dólares
        const cuentasUSD = cuentasResponse.filter(cuenta => cuenta.moneda === 'USD');
        setCuentasBancarias(cuentasUSD);

        // Cargar configuración actual
        const configResponse = await wepaUsdService.getLatestConfig();
        if (configResponse) {
        ...
        // Cargar historial
        const historialResponse = await wepaUsdService.getConfigHistory();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Guardar configuración
      const response = await wepaUsdService.createConfig(data);
  ...
  const handleDownloadContrato = async (id: number) => {
    try {
      const response = await wepaUsdService.downloadContrato(id);
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `contrato-wepa-usd-${id}.pdf`);
          ...
          <TableBody>
            {historialConfig.map((config) => (
              <TableRow key={config.id} sx={{ '&:hover': { backgroundColor: 'rgba(255, 255, 255, 0.08)' } }}>
                ...
                <TableCell sx={{ color: 'white' }}>{formatCurrency.dollars(Number(config.limiteCredito))}</TableCell>
                <TableCell sx={{ color: 'white' }}>
                  ...
                  {configActual ? (
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} md={2}>
                        ...
                        <Typography variant="body1">
                          {formatCurrency.dollars(configActual.limiteCredito)}
                        </Typography>
                      </Grid>
                    ...
                    <Typography variant="body1" sx={{ color: 'rgba(255, 255, 255, 0.7)' }}>
                      No hay configuración registrada para Wepa USD.
                    </Typography>
                  )}
                </CardContent>
              </Card>
              ...
              <Card sx={{ backgroundColor: '#1E1E1E', color: 'white', height: '100%' }}>
                <CardContent>
                  ...
                  <Alert severity="info" sx={{ mb: 2, color: 'black' }}>
                    La configuración de Wepa USD determina la cuenta bancaria donde se depositarán los fondos
                    y establece el límite de crédito en dólares para operar.
                  </Alert>
                    ...
                    <ul>
                      <li>
                        <Typography variant="body2">
                          El límite de crédito establece el monto máximo disponible para transferencias en dólares.
                        </Typography>
                      </li>
                      <li>
                        <Typography variant="body2">
                  ...
                  <Box sx={{ mt: 2 }}>
                    ...
                    <Button
                      variant="text"
                      color="primary"
                      onClick={() => window.location.href = '/dashboard/saldos-monetarios/wepa-usd'}
                    >
                      Ir a Balance Wepa USD
                    </Button>
                  </Box>
