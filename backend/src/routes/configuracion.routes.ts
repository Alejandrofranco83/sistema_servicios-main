...
import { 
    createAquipagoConfig, 
    getLatestAquipagoConfig, 
    getAquipagoConfigHistory,
    downloadContrato
} from '../controllers/aquipago.controller';
import authMiddleware from '../middleware/auth'; // Importación por defecto
...
router.post('/aquipago', upload.single('contratoFile'), createAquipagoConfig);
console.log('[DEBUG] Ruta POST /aquipago registrada');

// GET /api/configuracion/aquipago/actual - Obtener la última configuración
router.get('/aquipago/actual', getLatestAquipagoConfig);
...
router.get('/aquipago/historial', getAquipagoConfigHistory);
...
router.get('/aquipago/:id/contrato', downloadContrato);
...
router.use('/wepa-gs', require('../../routes/wepa-gs.routes')(authMiddleware));
...
router.use('/wepa-usd', require('../../routes/wepa-usd.routes')(authMiddleware));
