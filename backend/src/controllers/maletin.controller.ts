...
export const getMaletines = async (_req: Request, res: Response) => {
  console.log('Accediendo a getMaletines - inicio de función');
  try {
    const maletines = await prisma.maletin.findMany({
      include: {
        sucursal: {
          select: {
            id: true,
    ...
    const maletinesFormateados = maletines.map(maletin => ({
      ...maletin,
      id: String(maletin.id),
      sucursalId: String(maletin.sucursalId),
      sucursal: maletin.sucursal ? {
        ...maletin.sucursal,
        id: String(maletin.sucursal.id)
      } : undefined
    }));
...
export const getMaletinById = async (req: Request, res: Response) => {
  const { id } = req.params;

  try {
    const maletin = await prisma.maletin.findUnique({
...
export const createMaletin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const maletinExistente = await prisma.maletin.findFirst({
      where: { codigo }
    });
    ...
    const nuevoMaletin = await prisma.maletin.create({
      data: {
        codigo,
    ...
    const maletinRespuesta = {
      ...nuevoMaletin,
      id: String(nuevoMaletin.id),
      sucursalId: String(nuevoMaletin.sucursalId),
      sucursal: nuevoMaletin.sucursal ? {
        ...nuevoMaletin.sucursal,
        id: String(nuevoMaletin.sucursal.id)
      } : undefined
    };
...
export const updateMaletin = async (req: Request, res: Response) => {
  const { id } = req.params;
  const { codigo, sucursalId } = req.body;
  ...
  try {
    ...
    const maletinExistente = await prisma.maletin.findUnique({
      where: { id: parseInt(id) }
    });
    ...
    if (codigo && codigo !== maletinExistente.codigo) {
      const codigoExistente = await prisma.maletin.findFirst({
        where: {
          codigo,
          id: { not: parseInt(id) }
        }
    ...
    const maletinActualizado = await prisma.maletin.update({
      where: { id: parseInt(id) },
      data: {
    ...
    const maletinRespuesta = {
      ...maletinActualizado,
      id: String(maletinActualizado.id),
      sucursalId: String(maletinActualizado.sucursalId),
      sucursal: maletinActualizado.sucursal ? {
        ...maletinActualizado.sucursal,
        id: String(maletinActualizado.sucursal.id)
      } : undefined
    };
...
// Eliminar un maletín
export const deleteMaletin = async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  try {
    ...
    const cajasAsociadas = await prisma.caja.findFirst({
...
export const getMaletinesBySucursal = async (req: Request, res: Response) => {
