 ...
 */
export const WepaUsdController = {
  /**
   ...
   */
  getBalanceGlobal: async (req: Request, res: Response) => {
    try {
      ...
      const wepaUsdConfig = await prisma.wepaUsdConfig.findFirst({
        orderBy: {
          fechaCreacion: 'desc'
        }
      });
      ...
      const sumaDepositosGlobal = wepaUsdConfig?.cuentaBancariaId ? 
        await prisma.depositoBancario.aggregate({
          _sum: {
            monto: true,
          },
          where: {
      ...
      const totalPagosGlobal = sumaPagosGlobal._sum.monto?.toNumber() || 0;
      const totalRetirosGlobal = sumaRetirosGlobal._sum.monto?.toNumber() || 0;
      const totalDepositosGlobal = sumaDepositosGlobal._sum.monto?.toNumber() || 0;
  ...
  obtenerMovimientos: async (req: Request, res: Response) => {
    try {
      ...
      const depositos = (wepaUsdConfig?.cuentaBancariaId) ? await prisma.depositoBancario.findMany({
        where: {
          fecha: {
            gte: fechaInicioUTC,
            lt: fechaFinSiguienteUTC,
