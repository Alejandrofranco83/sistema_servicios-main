...
class UsoDevolucionModel {
    ...
    create(data) {
        return __awaiter(this, void 0, void 0, function* () {
            const { tipo, persona_id, persona_nombre, guaranies = 0, dolares = 0, reales = 0, motivo, usuario_id } = data;
            const guaraniesNum = BigInt(guaranies);
            const dolaresNum = new client_1.Prisma.Decimal(dolares);
            const realesNum = new client_1.Prisma.Decimal(reales);
            const fechaHoraActual = new Date(); // Usar la misma fecha/hora para todos los registros
            try {
                return yield prisma.$transaction((prismaClient) => __awaiter(this, void 0, void 0, function* () {
                    // 1. Crear registro usoDevolucion (sin cambios)
                    console.log('[DEBUG] Creando registro usoDevolucion...');
                    const resultado = yield prismaClient.usoDevolucion.create({
                        data: {
                            tipo,
                    ...
                    if (persona_id === FARMACIA_FRANCO_PERSONA_ID) {
                        ...
                        if (guaraniesNum !== BigInt(0)) {
                            const montoPYG = new library_1.Decimal(tipo === 'USO' ? Math.abs(Number(guaraniesNum)) : -Math.abs(Number(guaraniesNum)));
                            movimientosFarmaciaPromises.push(prismaClient.movimientoFarmacia.create({
                                data: {
                        ...
                        yield Promise.all(movimientosFarmaciaPromises);
                        ...
                        try {
                            ...
                            const upsertResult = yield prismaClient.saldoPersona.upsert({
                                where: { persona_id },
                                update: {
                    ...
                    try {
                        ...
                        if (Number(guaraniesNum) > 0) {
                            yield dbUtils_1.default.registrarMovimientoCajaMayor(resultado.id, 'guaranies', Number(guaraniesNum), esIngresoCMM, conceptoCMM, usuario_id, resultado.id);
                        }
                        if (dolaresNum.greaterThan(0)) {
                            yield dbUtils_1.default.registrarMovimientoCajaMayor(resultado.id, 'dolares', dolaresNum.toNumber(), esIngresoCMM, conceptoCMM, usuario_id, resultado.id);
                        }
                        if (realesNum.greaterThan(0)) {
                            yield dbUtils_1.default.registrarMovimientoCajaMayor(resultado.id, 'reales', realesNum.toNumber(), esIngresoCMM, conceptoCMM, usuario_id, resultado.id);
                        }
                    ...
                    return Object.assign(Object.assign({}, resultado), { guaranies: Number(resultado.guaranies) });
                })); // Fin Transacción
            }
            catch (error) {
    ...
    getAll() {
        return __awaiter(this, arguments, void 0, function* (filters = {}) {
            try {
                ...
                const operaciones = yield prisma.usoDevolucion.findMany({
                    where,
                    include: {
                        persona: true,
                        usuario: true
                ...
                return operaciones.map((op) => (Object.assign(Object.assign({}, op), { guaranies: Number(op.guaranies) })));
            }
            catch (error) {
    ...
    getById(id) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // @ts-ignore - Ignorar errores de tipos en Prisma
                const operacion = yield prisma.usoDevolucion.findUnique({
                    where: { id },
                    include: {
                        persona: true,
                        usuario: true
                ...
                if (!operacion)
                    return null;
                // Convertir BigInt a número para la respuesta
                return Object.assign(Object.assign({}, operacion), { guaranies: Number(operacion.guaranies) });
            }
            catch (error) {
     ...
     */
    getSaldoPersona(persona_id) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // @ts-ignore - Ignorar errores de tipos en Prisma
                const saldo = yield prisma.saldoPersona.findUnique({
                    where: { persona_id }
                });
                if (!saldo) {
                ...
                return Object.assign(Object.assign({}, saldo), { guaranies: Number(saldo.guaranies) });
            }
            catch (error) {
    ...
    anular(id, usuario_id) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const resultado = yield prisma.$transaction((tx) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    try {
                        ...
                        if (Number(operacion.guaranies) > 0) {
                            yield dbUtils_1.default.registrarMovimientoCajaMayor(id, 'guaranies', Number(operacion.guaranies), esIngresoAnulacion, // Usar lógica invertida correcta para anulación
                            `Anulación de ${operacion.tipo === 'USO' ? 'préstamo de' : 'devolución a'} ${operacion.persona_nombre} ID: ${id}`, usuario_id, id);
                        }
                        if (Number(operacion.dolares) > 0) {
                            yield dbUtils_1.default.registrarMovimientoCajaMayor(id, 'dolares', Number(operacion.dolares), esIngresoAnulacion, // Usar lógica invertida correcta para anulación
                            `Anulación de ${operacion.tipo === 'USO' ? 'préstamo de' : 'devolución a'} ${operacion.persona_nombre} ID: ${id}`, usuario_id, id);
                        }
                        if (Number(operacion.reales) > 0) {
                            yield dbUtils_1.default.registrarMovimientoCajaMayor(id, 'reales', Number(operacion.reales), esIngresoAnulacion, // Usar lógica invertida correcta para anulación
                            `Anulación de ${operacion.tipo === 'USO' ? 'préstamo de' : 'devolución a'} ${operacion.persona_nombre} ID: ${id}`, usuario_id, id);
                        }
                    }
                    catch (error) {
     ...
     */
    getHistorialPersona(persona_id) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // @ts-ignore - Ignorar errores de tipos en Prisma
                const historial = yield prisma.usoDevolucion.findMany({
                    where: {
                        persona_id,
                        anulado: false
                    },
                ...
                // Convertir BigInt a número para la respuesta
                return historial.map((op) => (Object.assign(Object.assign({}, op), { guaranies: Number(op.guaranies) })));
            }
            catch (error) {
     ...
     */
    getByMovimientoId(movimientoId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                // @ts-ignore - Ignorar errores de tipos en Prisma
                const operacion = yield prisma.usoDevolucion.findFirst({
                ...
                // Convertir BigInt a número para la respuesta
                return Object.assign(Object.assign({}, operacion), { guaranies: Number(operacion.guaranies), cajaMayorMovimiento: movimiento });
            }
            catch (error) {
     ...
     */
    cancelar(id_1, usuario_id_1) {
        return __awaiter(this, arguments, void 0, function* (id, usuario_id, razon = 'Cancelación') {
            try {
                ...
                return yield prisma.$transaction((tx) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    try {
                        ...
                        const concepto = `Cancelación de ${operacion.tipo === 'USO' ? 'préstamo de' : 'devolución a'} ${((_a = operacion.persona) === null || _a === void 0 ? void 0 : _a.nombreCompleto) || 'persona'} ID: ${operacion.id}`;
                        ...
                        // @ts-ignore - Ignorar errores de tipos en Prisma
                        const ultimoMovGuaranies = yield tx.cajaMayorMovimiento.findFirst({
                        ...
                        // @ts-ignore - Ignorar errores de tipos en Prisma
                        const ultimoMovDolares = yield tx.cajaMayorMovimiento.findFirst({
                        ...
                        // @ts-ignore - Ignorar errores de tipos en Prisma
                        const ultimoMovReales = yield tx.cajaMayorMovimiento.findFirst({
                            where: { moneda: 'reales' },
                            orderBy: { id: 'desc' },
                            select: { saldoActual: true }
                        });
                        ...
                        // @ts-ignore - Ignorar errores de tipos en Prisma
                        const resultadosMovimientos = yield Promise.all(movimientos.map(mov => tx.cajaMayorMovimiento.create({ data: mov })));
                    ...
                    // @ts-ignore - Ignorar errores de tipos en Prisma
                    const saldoActualizado = yield tx.saldoPersona.findUnique({
                        where: { persona_id: operacion.persona_id }
                    });
                    return {
                        success: true,
                        ...
                        data: {
                            operacionId: id,
                            tipo: operacion.tipo,
                            movimientoId: movimientoNuevo === null || movimientoNuevo === void 0 ? void 0 : movimientoNuevo.id,
    ...
    getPersonasConSaldo() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // @ts-ignore - Ignorar errores de tipos en Prisma
                const saldos = yield prisma.saldoPersona.findMany({
                    where: {
                        OR: [
                            { guaranies: { not: 0 } },
                            { dolares: { not: 0 } },
                ...
                return saldos.map(saldo => (Object.assign(Object.assign({}, saldo), { guaranies: Number(saldo.guaranies), nombre_completo: saldo.persona.nombreCompleto, documento: saldo.persona.documento, 
...
exports.default = new UsoDevolucionModel();
//# sourceMappingURL=usoDevolucionModel.js.map